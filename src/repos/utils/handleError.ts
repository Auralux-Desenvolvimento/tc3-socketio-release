...
import parseClassValidatorErrors from "../../utils/parseClassValidatorErrors";

export default function handleError (error: any, dbHandler?: (error: QueryFailedError & { errno: number }) => undefined|Error) {
  debugger;
  if (process.env.NODE_ENV !== "allow-all-errors") {
    ...
    switch (error.name) {
      case "QueryFailedError":
        ...
        throw new AppError([400, "DB_Error"], 0, error.errno);
      case "AppError":
        throw error;
      default:
